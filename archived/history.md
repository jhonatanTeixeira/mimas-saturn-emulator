...
## Chapter 44 — Why an empty drive answers PAUSE at reset (and why that alone did not move the boot)

The real boot reads the CD block status as `0x21` (PERI|PAUSE) at reference instruction #496; ours read `0x27`. `reset_system()` short-circuited an empty drive straight to NODISC, whereas the reference resets to PAUSE unconditionally and only a periodic backend poll (`_statustiming` = 1,000,000 / 3 = 333,333 µs of emulated time, `cs2.c:964`) moves an empty drive to NODISC. Mimas now resets to PAUSE and leaves the existing 3 Hz poll to do the rest.

**Measured result: the ordered boot did not move** (no disc: still gap at frame 180 / `0x000033D8`; with the fixture disc: unchanged). The BIOS's first CR1 read still returns `0x27` because our poll fires *before* that read (frame 181 is ~3 s of emulated time, the poll is 333 ms). A throwaway experiment that pushed the poll period to 333 s made the first read `0x21` and executed both `0x000033DC` and `0x000032DC`. So the reset change is necessary but not sufficient: the real capture must see its first poll later than ours does (or an empty drive is not polled to NODISC before that read). Cause not yet identified; do not shorten or lengthen the period without evidence from the reference trace (candidate: how much emulated time `exec_vblank` credits versus the reference's `cdb_accum_t`, `yabause.c:835`).
