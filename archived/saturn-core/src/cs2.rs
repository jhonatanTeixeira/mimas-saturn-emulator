...
mod tests {
    ...
    fn test_cs2_handshake_cr4_write_exec() {
        ...
        assert_eq!(cs2.cr1, 0x0100); // PAUSE without disc, not NODISC
    ...
    fn test_phase2_command_handshake_and_get_hardware_info() {
        ...
        assert!(!cs2.command_pending);
        assert_eq!(cs2.hirq & HIRQ_CMOK, HIRQ_CMOK);
        assert_eq!(cs2.cr1 >> 8, CDB_STAT_PAUSE as u16);
    ...
    #[test]
    fn cd_block_reset_to_pause_not_nodisc_without_disc() {
        let mut cs2 = Cs2::new();
        assert_eq!(
            cs2.status, CDB_STAT_PAUSE,
            "reset_system() must return PAUSE even without a disc"
        );
    }

    #[test]
    fn cd_block_empty_drive_stays_pause_before_first_status_poll() {
        let mut cs2 = Cs2::new();
        cs2.exec(100_000);
        assert_eq!(
            cs2.status & 0x0F,
            CDB_STAT_PAUSE,
            "before 333,333 µs, empty drive stays PAUSE"
        );
    }

    #[test]
    fn cd_block_empty_drive_transitions_to_nodisc_after_status_period() {
        let mut cs2 = Cs2::new();
        cs2.exec(333_333);
        assert_eq!(
            cs2.status & 0x0F,
            CDB_STAT_NODISC,
            "after 333,333 µs, empty drive transitions to NODISC"
        );
    }

    #[test]
    fn cd_block_with_disc_stays_pause_throughout() {
        let mut cs2 = Cs2::new();
        let fixture_path = "tests/fixtures/single_data_track.chd";
        if !std::path::Path::new(fixture_path).exists() {
            return;
        ...
        assert_eq!(
            cs2.status & 0x0F,
            CDB_STAT_PAUSE,
            "after load_disc, status is PAUSE"
        );
        ...
        assert_eq!(
            cs2.status & 0x0F,
            CDB_STAT_PAUSE,
            "with disc present, periodic status checks do not change to NODISC"
        );
    }
