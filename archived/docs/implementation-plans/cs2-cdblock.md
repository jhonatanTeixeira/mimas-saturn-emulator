...
**Status (power-on status):** **Partial** — reset now answers PAUSE with no disc (plan `unlock_bios/04`), but the 3 Hz poll still turns it NODISC before the BIOS's first read; see `history.md` Chapter 44.
