...
| 7 | `smpc-cd-block` | SMPC command dispatch and CD-block logic. Parks when idle; woken exactly when Master SH-2's own cycle-driven checkpoints confirm an armed SMPC command's real delay elapsed, or when CD-block has work. |
...
- **Only Cores 0 and 1 loop continuously — every other core is genuinely parked-until-woken.** This is a hard project rule (`docs/mimas-architecture-spec.md` §1.4/§1.5), not a preference: the *only* continuous loop anywhere in the system is the CPU's own (`Sh2::run_loop`, both Master and Slave), paced against real Saturn clock rates in cycles-per-Hz terms (`ClockThrottle`, batched wall-clock pacing — §1.4). Every other component thread (2, 3, 4, 6, 7) parks via `LockStepSync::park_while_inactive` and is woken only by a real hardware-equivalent event — a register write, an interrupt, or (for Core 3) Master SH-2's own cycle-driven video timing. **Core 5 (SCSP) breaks this rule today and is not excused for it.** It never reaches `park_while_inactive`, so `tools/golden_rules.py` fails on it — deliberately with no escape hatch: `rule_spawned_threads_park` takes no allowlist entry and no `// golden-rule-ok:` marker for it. The old justification ("real hardware synthesizes audio continuously regardless of what any CPU is doing") is true but does not single Core 5 out: real VDP2 scans out pixels continuously too, and Core 3 still parks, woken at the cycle-driven V-Blank IN moment. That argument is for advancing *emulated state* continuously, not for burning a host thread. Measured cost of leaving it: 0.564 s of CPU across a 2.33 s BIOS boot, ~24% of one core, synthesizing near-certain silence — invisible on a desktop, a quarter of a core the R36S does not have. The fix is the Core 3 pattern: wake in batches on the Master's cycle-driven schedule. Core 2 has zero real work in the current implementation at all (VDP1 execution actually runs inline from Master SH-2's cycle-driven timing via Core 3, not Core 2) and parks unconditionally, forever, with nothing yet wiring a wake call to it. Core 7 dispatches SMPC commands (`Smpc::execute_expired_command`, gated on `Smpc::is_dispatch_ready`, which only Master's own cycle-driven `step()` may set — see `docs/implementation-plans/smpc-peripheral.md` Phase 6) and CD-block logic, parking whenever neither has real work to do right now.
