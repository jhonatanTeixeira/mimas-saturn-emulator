...
def repl_gen(m):
    body = m.group(1)
    for var in ["mpab", "mpcd", "mpofn", "patterndatasize", "patternwh", "planew", "planeh", "vram_8mbit"]:
        body = re.sub(r'\b' + var + r'\b', 'cfg.' + var, body)
    return r'pub fn generate_plane_addr_table(planetbl: &mut [u32; 4], cfg: &Vdp2LayerConfig) {' + body + '\n}'

text = re.sub(
    r'pub fn generate_plane_addr_table\(planetbl: &mut \[u32; 4\], cfg: &Vdp2LayerConfig\) \{([\s\S]*?)^\}',
    repl_gen,
    text,
    flags=re.MULTILINE
)
...
def repl_map(m):
    body = m.group(1)
    for var in ["patternwh", "mapwh"]:
        body = re.sub(r'\b' + var + r'\b', 'cfg.' + var, body)
    
    body = re.sub(
        r'pattern_addr\(\s*tmp1,\s*tmp2,\s*supplementdata,\s*auxmode,\s*cfg\.patternwh,\s*patterndatasize,\s*colornumber,\s*vram_8mbit,\s*\)',
        r'pattern_addr(tmp1, tmp2, cfg)',
        body
    )
    body = re.sub(
        r'pattern_addr\(\s*tmp\[0\],\s*tmp\[1\],\s*supplementdata,\s*auxmode,\s*cfg\.patternwh,\s*patterndatasize,\s*colornumber,\s*vram_8mbit,\s*\)',
        r'pattern_addr(tmp[0], tmp[1], cfg)',
        body
    )
    return r'pub fn map_calc_xy(state: &mut Vdp2State, x: u32, y: u32, vars: &ScreenVars, cfg: &Vdp2LayerConfig, vram: &[u8]) {' + body + '\n}'

text = re.sub(
    r'pub fn map_calc_xy\(state: &mut Vdp2State, x: u32, y: u32, vars: &ScreenVars, cfg: &Vdp2LayerConfig, vram: &\[u8\]\) \{([\s\S]*?)^\}',
    repl_map,
    text,
    flags=re.MULTILINE
)
...
def repl_pat(m):
    body = m.group(1)
    for var in ["patternwh", "vram_8mbit", "patterndatasize", "colornumber", "auxmode", "supplementdata"]:
        body = re.sub(r'\b' + var + r'\b', 'cfg.' + var, body)
    return r'pub fn pattern_addr(tmp1: u16, tmp2: u16, cfg: &Vdp2LayerConfig) {' + body + '\n}'

text = re.sub(
    r'pub fn pattern_addr\(tmp1: u16, tmp2: u16, cfg: &Vdp2LayerConfig\) \{([\s\S]*?)^\}',
    repl_pat,
    text,
    flags=re.MULTILINE
)
...
def repl_fetch(m):
    body = m.group(1)
    for var in ["patternwh", "colornumber", "transparencyenable", "cram_mode", "coloroffset"]:
        body = re.sub(r'\b' + var + r'\b', 'cfg.' + var, body)
    return r'pub fn fetch_pixel(charaddr: u32, paladdr: u32, mut x: u32, mut y: u32, flipfunction: u16, cellw: u32, cfg: &Vdp2LayerConfig, vram: &[u8], cram: &[u8]) {' + body + '\n}'

text = re.sub(
    r'pub fn fetch_pixel\(charaddr: u32, paladdr: u32, mut x: u32, mut y: u32, flipfunction: u16, cellw: u32, cfg: &Vdp2LayerConfig, vram: &\[u8\], cram: &\[u8\]\) \{([\s\S]*?)^}',
    repl_fetch,
    text,
    flags=re.MULTILINE
)
